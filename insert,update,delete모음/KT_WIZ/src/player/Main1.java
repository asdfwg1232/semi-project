...
//					-----------------------------------------------------------
			...
			case "5" :	
				...
				if(str5.equals("타자")) {
					...
					try {
						List<Pitcherplayer> playerList = dao.PlayerYars_pitcherFindAll();
						for(Pitcherplayer player : playerList) {
						}
						dao.writePitcherplayerFile(filepath, filename, playerList);
			...
			case "6" :
				...
				while(true) {
					if(ans.equals("타자")) {
						try {
							pi.setInfoList_Batter(dao.readFile_batter(new File(filepath1, filename1)));
							System.out.println(pi.infoList_batter+"\n");
						}catch(ClassNotFoundException e) {
							e.printStackTrace();
						} catch (IOException e) {
							// TODO Auto-generated catch block
							...
							try {
								pi.setInfoList_pitcher(dao.readFile_pitcher(new File(filepath1, filename1)));
								System.out.println(pi.infoList_pitcher+"\n");
							}catch(ClassNotFoundException e) {
								e.printStackTrace();
							} catch (IOException e) {
								// TODO Auto-generated catch block
